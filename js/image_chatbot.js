// ============================================================
// image_chatbot.js
// S.N.E.T.C.H Image Chatbot — full production logic
// Talks to Flask API mounted at /image_chatbot/api/*
// ============================================================

(function () {
  'use strict';

  const API_BASE = '/image_chatbot/api';
  const ALLOWED_EXT = ['png', 'jpg', 'jpeg', 'webp', 'bmp', 'gif'];
  const MAX_BYTES = 20 * 1024 * 1024;
  const ONLY_IMAGE_MSG = 'Only image files are supported.';
  ...
  const imgUploadTime = document.getElementById('imgUploadTime');
  ...
  const replaceImgBtn = document.getElementById('replaceImgBtn');
  ...
  function imageUrl(threadId) {
    return `${API_BASE}/thread/${threadId}/image?t=${Date.now()}`;
  }
  ...
  function renderChatList(threads) {
    ...
    threads.forEach((t) => {
      ...
      const icon = t.image ? 'fa-image' : 'fa-comment-dots';
      li.innerHTML = `
        <div class="chat-info">
          <i class="fas ${icon}"></i>
          <div class="chat-meta">
  ...
  async function startNewChat() {
    try {
      ...
      toast('Could not start a new image chat: ' + e.message, true);
    }
  }
  ...
  function isImageFile(file) {
    ...
    const mimeOk = (file.type || '').toLowerCase().startsWith('image/');
    return ALLOWED_EXT.includes(ext) && mimeOk;
  }
  ...
  async function handleFile(file) {
    ...
    if (!isImageFile(file)) {
      showUploadError(ONLY_IMAGE_MSG);
      return;
    }
    if (file.size > MAX_BYTES) {
      showUploadError('Image exceeds the 20MB limit.');
      return;
    }

    uploadError.classList.add('hidden');
    setProgress(15, 'Uploading image...');

    const formData = new FormData();
    formData.append('thread_id', currentThreadId);
    formData.append('file', file);

    // Instant local preview while the server processes the image.
    const localPreviewUrl = URL.createObjectURL(file);

    try {
      setProgress(45, 'Validating image...');
      const data = await api('/upload', { method: 'POST', body: formData });
      setProgress(85, 'Preparing image for analysis...');
      currentThread = data.thread;
      await new Promise((r) => setTimeout(r, 250));
      setProgress(100, 'Ready to chat');
      await new Promise((r) => setTimeout(r, 300));
      uploadProgress.classList.add('hidden');
      enterChatMode(localPreviewUrl);
      loadThreadList();
    } catch (e) {
      uploadProgress.classList.add('hidden');
      showUploadError(e.message || ONLY_IMAGE_MSG);
    }
  }
  ...
  async function openThread(threadId) {
    try {
      ...
      if (data.thread.processing_status === 'ready') {
        enterChatMode(imageUrl(threadId));
  ...
  function enterChatMode(previewUrl) {
    ...
    renderImgCard(previewUrl);
    if (!messageContainer.children.length) {
      messageContainer.innerHTML = `
        <div class="welcome-msg">
          <i class="fas fa-sparkles"></i>
          Image ready! Ask me anything — describe it, read text inside it, count objects, or explain a chart.
        </div>`;
    }
  }

  function renderImgCard(previewUrl) {
    const img = currentThread && currentThread.image;
    if (!img) return;
    imgThumb.src = previewUrl || imageUrl(currentThreadId);
    imgName.textContent = img.name;
    imgType.textContent = (img.type || '').toUpperCase();
    imgSize.textContent = fmtBytes(img.size);
    imgUploadTime.textContent = fmtDate(img.upload_time);
  ...
  function setInputEnabled(enabled) {
    chatInput.disabled = !enabled || !isImageReady;
    sendBtn.disabled = !enabled || !isImageReady;
  }
  ...
  async function sendMessage() {
    if (!isImageReady || isStreaming || !currentThreadId) return;
  ...
  homeBtn.addEventListener('click', (e) => {
    // Home link points to / for full navigation; nothing else to do here.
  });
  ...
  // ============================================================
  // UPLOAD EVENTS (Browse Image + Drag & Drop — images only)
  // ============================================================
  ...
  replaceImgBtn.addEventListener('click', () => startNewChat());
